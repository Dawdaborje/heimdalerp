...
from core.models import TimeStampedModel, UserTrackingModel, OrganizationModel
...
class ModuleModel(TimeStampedModel, UserTrackingModel, OrganizationModel):
    ...
    version_validator = RegexValidator(
        regex=r'^\d+\.\d+(\.\d+)?$',
        message=_('Version must be in format X.Y or X.Y.Z')
    )
    ...
    total_installations = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Total Installations')
    )
    ...
    def is_compatible(self, system_version: str) -> bool:
        ...
        try:
            ...
            return version.parse(system_version) >= version.parse(self.min_system_version)
        except ImportError:
    ...
    @classmethod
    def get_modules_by_category(cls, category: Optional[ModuleCategory] = None) -> models.QuerySet:
        """
